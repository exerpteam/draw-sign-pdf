<template>
    <div>
        <main class="flex flex-col items-center py-16 bg-gray-100 min-h-screen">
            <div class="fixed z-10 top-0 left-0 right-0 h-12 flex justify-center items-center
          bg-gray-200 border-b border-gray-300">
                <input type="file" name="pdf" id="pdf" @change="onUploadPDF" class="hidden" />
                <label class="whitespace-no-wrap bg-blue-500 hover:bg-blue-700 text-white
            font-bold py-1 px-3 md:px-4 rounded mr-3 cursor-pointer md:mr-4" for="pdf">
                    Choose PDF
                </label>
                <div class="relative mr-3 flex h-8 bg-gray-400 rounded-sm overflow-hidden
            md:mr-4">
                    <label class="flex items-center justify-center h-full w-8 hover:bg-gray-500
              cursor-pointer" @click="onAddDrawing"
                        :class="{ 'cursor-not-allowed': selectedPageIndex < 0, 'bg-gray-500': selectedPageIndex < 0 }">
                        <img src="gesture.png" alt="An icon for adding drawing" />
                    </label>
                </div>
                <button @click="savePDF" class="w-20 bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-3
            md:px-4 mr-3 md:mr-4 rounded"
                    :class="{ 'cursor-not-allowed': pages.length === 0 || saving || !pdfFile, 'bg-blue-700': pages.length === 0 || saving || !pdfFile }">
                    {{ saving ? 'Saving' : 'Save' }}
                </button>
            </div>
            <div transition:fly="{ y: -200, duration: 500 }" class="fixed z-10 top-0 left-0 right-0 border-b border-gray-300 bg-white
          shadow-lg" style="height: 50%;" v-if="addingDrawing">
            ...
            <div class="w-full" v-if="pages.length">
                <div v-for="(page, pIndex) in  pages " :key="pIndex"
                    class="p-5 w-full flex flex-col items-center overflow-hidden" @mousedown="selectPage(pIndex)"
                    ...
                    <div class="relative shadow-lg" :class="{ 'shadow-outline': pIndex === selectedPageIndex }">
                        ...
                        <div class="absolute top-0 left-0 transform origin-top-left"
                            :style="{ transform: `scale(${pagesScale[pIndex]})`, touchAction: 'none' }">
                            <div v-for="object in  allObjects[pIndex] " :key="object.id">
                                ...
                                @delete="deleteObject" /> -->
                                <Drawing v-if="object.type === 'drawing'"
                                    @update="(e) => updateObject(object.id, e)"
                                    @delete="() => deleteObject(object.id)" :path="object.path" :x="object.x"
                                    :y="object.y" :width="object.width" :originWidth="object.originWidth"
                                    :originHeight="object.originHeight" :pageScale="pagesScale[pIndex]" />
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-full flex-grow flex justify-center items-center" v-else>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const updateObject = (objectId, payload) => {
            console.log(objectId, payload, "hit", allObjects.value, selectedPageIndex.value);
