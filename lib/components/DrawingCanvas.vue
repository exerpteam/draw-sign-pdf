    ...
    <div style="height: 50%" class="fixed z-10 top-0 left-0 right-0 border-b border-gray-300 bg-white shadow-lg">
        <div ref="signatureCanvas" @panstart="handlePanStart" @panmove="handlePanMove" @panend="handlePanEnd"
            ...
            <div class="absolute right-0 bottom-0 mr-4 mb-4 flex">
                <button @click="cancel"
                    class="w-24 bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-4 rounded mr-4">Cancel</button>
                <button @click="finish"
                    class="w-24 bg-blue-600 hover:bg-blue-700 text-white font-bold py-1 px-4 rounded">Done</button>
            </div>
            <svg class="w-full h-full pointer-events-none">
                <path stroke-width="5" stroke-linejoin="round" stroke-linecap="round" :d="path" stroke="black"
                    fill="none" />
            </svg>
        </div>
    </div>
